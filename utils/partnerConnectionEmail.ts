...
export function connectionsWebLink(requestId?: string): string {
  const base = `${frontendBase()}/connections`;
  if (!requestId) return base;
  return `${base}?requestId=${encodeURIComponent(requestId)}`;
}

export function connectionsDeepLink(requestId?: string): string {
  const scheme = deepLinkScheme();
  if (!requestId) return `${scheme}://connections`;
...
function senderProfileLines(sender: UserWithoutPassword): string {
  const parts: string[] = [];
...
export async function sendPartnerConnectRequestEmail(
  recipient: UserWithoutPassword,
  sender: UserWithoutPassword,
  requestId: string
): Promise<{ success: boolean; skipped?: boolean }> {
  if (!recipient.email?.trim()) return { success: false, skipped: true };
...
export async function sendPartnerConnectAcceptedEmail(
  originalSender: UserWithoutPassword,
  accepter: UserWithoutPassword
): Promise<{ success: boolean; skipped?: boolean }> {
  if (!originalSender.email?.trim()) return { success: false, skipped: true };
