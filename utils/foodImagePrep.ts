...
import { ClarifaiServiceError, getFoodScanMaxBase64Chars } from '../services/clarifaiClient.ts';

function getMaxEdgePx(): number {
  return Number(process.env.FOOD_SCAN_MAX_EDGE_PX || 1280);
}

function getInitialJpegQuality(): number {
 ...
 */
export async function prepareFoodScanBase64(cleanBase64: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    try {
      ...
      throw new ClarifaiServiceError(
        'Could not process image. Use a JPEG or PNG photo.',
        422
      );
    }
  ...
  throw new ClarifaiServiceError(
    `Image is still too large after compression. Maximum is about ${Math.round(maxChars / 1024)}KB. Try a smaller photo.`,
    413
  );
}
