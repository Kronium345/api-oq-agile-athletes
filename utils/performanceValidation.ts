import type { CheckInInputs } from '../services/performanceScoring.ts';

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

export function isValidPerformanceDate(date: string): boolean {
...
export function validateCheckInBody(body: Record<string, unknown>): {
  ok: true;
  inputs: CheckInInputs & { date: string };
  ...
  let proteinIntake: number | undefined;
  ...
  let waterIntakeLiters: number | undefined;
  if (
    body.waterIntakeLiters !== undefined &&
