...
export interface FitnessGroupSeedResult {
  legacyRemoved: number;
  upserted: number;
  groupCount: number;
}

export async function ensureFitnessGroupsSeeded(): Promise<FitnessGroupSeedResult> {
  ...
  if (removedLegacy.deletedCount > 0) {
    console.log(
      `[groups] removed ${removedLegacy.deletedCount} legacy fictional fitness group(s)`
    );
  }
