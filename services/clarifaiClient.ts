...
export class ClarifaiServiceError extends Error {
  ...
  constructor(message: string, statusCode: number, clarifaiStatus?: unknown) {
    super(message);
...
export function getClarifaiApiKey(): string {
  return process.env.FitnessOnePAT || process.env.CLARIFAI_API_KEY || '';
}

export function mapClarifaiAxiosError(error: unknown): ClarifaiServiceError {
  const axiosErr = error as AxiosError<{ status?: { description?: string; details?: string } }>;
  const status = axiosErr.response?.status;
  const clarifaiStatus = axiosErr.response?.data?.status;
  const detail =
    clarifaiStatus?.description ||
  ...
  if (status === 402) {
    return new ClarifaiServiceError(
      'Clarifai returned Payment Required (402). Your account has no credits or billing for the food recognition model. Add a payment method or upgrade your plan at clarifai.com, or use a new Personal Access Token with an active account.',
      402,
      clarifaiStatus
    );
  }
  ...
  if (status === 401 || status === 403) {
    return new ClarifaiServiceError(
      'Clarifai authentication failed. Check FitnessOnePAT / CLARIFAI_API_KEY on Render.',
      status,
      clarifaiStatus
    );
  }
  ...
  if (status === 429) {
    return new ClarifaiServiceError(
      'Clarifai rate limit exceeded. Try again later or upgrade your plan.',
      429,
      clarifaiStatus
    );
  }
  ...
  return new ClarifaiServiceError(
    `Clarifai request failed${status ? ` (${status})` : ''}: ${detail}`,
    status || 502,
    clarifaiStatus
  );
}
...
export async function postClarifaiJson<T>(url: string, data: unknown): Promise<T> {
  const apiKey = getClarifaiApiKey();
  if (!apiKey) {
    throw new ClarifaiServiceError(
      'Clarifai API key is not configured. Set FitnessOnePAT on Render.',
      503
    );
  }
  ...
  try {
    const response = await axios.post<T>(url, data, {
      ...
      timeout: Number(process.env.CLARIFAI_TIMEOUT_MS || 60_000),
