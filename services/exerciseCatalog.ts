...
export interface CatalogRateLimitError {
  status: number;
  message: string;
  data?: unknown;
}
...
export function mapExerciseToEnhanced(exercise: RawExercise): EnhancedExercise {
...
function rateLimitErrorFromStatus(status: number, data?: unknown): CatalogRateLimitError | null {
  if (status === 429) {
    return {
      status: 429,
 ...
 */
function buildPrimaryUrl(query: ExerciseCatalogQuery): string {
  const params = `limit=${UPSTREAM_FETCH_CAP}&offset=0`;
  const enc = (value: string) => encodeURIComponent(value.trim().toLowerCase());
...
function matchesFilter(value: string | undefined, expected?: string): boolean {
 ...
 */
export async function searchExerciseCatalog(query: ExerciseCatalogQuery): Promise<CatalogSearchResult> {
  ...
  const filtered = raw.filter(
    (exercise) =>
      matchesFilter(exercise.bodyPart, query.bodyPart) &&
