...
import { QUIZ_KEY_ORDER, REQUIRED_QUIZ_KEYS } from '../utils/quizValidation.ts';

const EXPECTED_QUESTION_COUNT = REQUIRED_QUIZ_KEYS.length;

const DEFAULT_QUESTIONS = (quizQuestionsData as QuizQuestion[]).map((q) => ({
  ...q,
  selected: null,
}));
...
function sortQuestions(questions: QuizQuestion[]): QuizQuestion[] {
...
function categoriesAreValid(categories: QuizCategory[]): boolean {
  if (categories.length !== DEFAULT_QUIZ_CATEGORIES.length) return false;
  return DEFAULT_QUIZ_CATEGORIES.every((expected) =>
    categories.some(
      (c) => c.label === expected.label && c.category === expected.category
    )
  );
}

function questionsAreValid(questions: QuizQuestion[]): boolean {
  if (questions.length !== EXPECTED_QUESTION_COUNT) return false;
  const names = new Set(questions.map((q) => q.name));
  return REQUIRED_QUIZ_KEYS.every((key) => names.has(key));
}

export async function ensureQuizCategoriesSeeded(): Promise<{ seeded: boolean; count: number; repaired: boolean }> {
  ...
  if (existing.length > 0) {
    console.warn(
      '[quiz] outcome categories invalid or legacy (e.g. Low Anxiety) — replacing with 4-class matrix'
    );
...
export async function ensureQuizQuestionsSeeded(): Promise<{ seeded: boolean; count: number; repaired: boolean }> {
  ...
  if (all.length > 0) {
    console.warn(
      `[quiz] questions invalid (expected ${EXPECTED_QUESTION_COUNT}, got ${all.length}) — reseeding from data/quizQuestions.json`
    );
