...
import { getStepHistory, getStepsByDate, markGoalAchieved } from '../models/stepHistory.ts';
import { getUserById, updateUser } from '../models/user.ts';
import { getLeaderboard } from './stepsSocial.ts';
import { resolveEmailNotifications } from '../utils/emailNotifications.ts';
...
export async function fetchUserForWorkflow(userId: string) {
  return getUserById(userId);
}

export function userWantsEmail(
  user: Awaited<ReturnType<typeof getUserById>>,
  key: keyof ReturnType<typeof resolveEmailNotifications>
): boolean {
...
export async function getTodayStepData(userId: string) {
...
export async function getWeeklyStepData(userId: string) {
  const today = todayUtc();
...
export async function getLeaderboardCompetitor(userId: string) {
  const { entries } = await getLeaderboard(userId, 'today', 'friends', 20);
  const self = entries.find((e) => e.userId === userId);
  if (!self) return { closeCompetitor: null as null | { name: string; stepsBehind: number } };
  ...
  if (!ahead) return { closeCompetitor: null };
...
export async function checkGoalAchievements(userId: string) {
  ...
  const dailyGoal = user.dailyStepGoal ?? 8000;
  const stepHistory = await getStepsByDate(userId, today);
  if (!stepHistory || stepHistory.stepCount < dailyGoal) return [];

  if (stepHistory.goalAchieved) return [];
...
export async function calculateGoalStreakDays(userId: string, dailyGoal: number): Promise<number> {
  let streak = 0;
  let cursor = todayUtc();
...
export async function updateUserLastMotivation(userId: string) {
  await updateUser(userId, { lastMotivationEmail: new Date().toISOString() });
}

export function displayNameForEmail(user: NonNullable<Awaited<ReturnType<typeof getUserById>>>) {
  return getDisplayName(user) || user.username || user.email;
}

export function fitnessUserPayload(user: NonNullable<Awaited<ReturnType<typeof getUserById>>>) {
  return {
