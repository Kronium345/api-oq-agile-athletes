...
export async function getOrCreateConnectAccount(profile: TrainerProfile): Promise<string> {
...
export async function syncConnectAccountFromWebhook(account: Stripe.Account): Promise<void> {
...
export async function createBookingPaymentIntent(params: {
  amountPence: number;
  trainerConnectAccountId: string;
  bookingId: string;
  trainerId: string;
  memberId: string;
}): Promise<{ paymentIntentId: string; clientSecret: string; platformFeePence: number }> {
  const stripe = getStripe();
  if (!stripe) throw new Error('Stripe is not configured');

  const platformFeePence = Math.round(
    (params.amountPence * getPlatformCommissionPercent()) / 100
  );
