...
export type RecommendationType = 'sleep' | 'stress' | 'nutrition' | 'training' | 'lifestyle';
...
const TRAINING_LOAD_SCORES: Record<TrainingLoadBand, number> = {
  Normal: 100,
  Building: 85,
  High: 65,
  'Very High': 40,
...
export function computeSleepScore(sleepHours: number, sleepQuality: number): number {
...
export function computeStressScore(stress: number): number {
  return (10 - stress) * 10;
}

export function computeEnergyScore(energy: number): number {
  return energy * 10;
}

export function computeSorenessScore(muscleSoreness: number): number {
  return (10 - muscleSoreness) * 10;
}

export function computeStepsScore(todaySteps: number, dailyGoal = 10000): number {
  if (dailyGoal <= 0) return 0;
  return clamp(todaySteps / dailyGoal, 0, 1) * 100;
}

export function computeLifestyleScore(inputs: CheckInInputs): number {
  let score = 50;

  if (inputs.proteinIntake != null) {
    if (inputs.proteinIntake >= 80) score += 25;
    else score -= 10;
  }

  if (inputs.waterIntakeLiters != null) {
    if (inputs.waterIntakeLiters >= 2) score += 25;
    else score -= 5;
  }
...
export function trainingLoadFromRatio(ratio: number): TrainingLoadBand {
...
export function computeAcwrRatio(acuteLoad: number, chronicLoad: number): number {
