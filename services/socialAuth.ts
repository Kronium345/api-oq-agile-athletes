...
import type { GoogleProfile } from './googleAuth.ts';
import type { AppleProfile } from './appleAuth.ts';

export function issueSessionToken(userId: string, email: string): string {
  return signAuthToken(userId, email);
}

export function buildSocialAuthResponse(user: UserWithoutPassword, isNewUser: boolean) {
...
export async function loginOrRegisterGoogle(
...
): Promise<{ user: UserWithoutPassword; isNewUser: boolean }> {
  ...
  if (user) {
    ...
    if (!user.lastName && profile.familyName) updates.lastName = profile.familyName;
  ...
  const lastName =
    profile.familyName || profile.name?.split(' ').slice(1).join(' ') || '';
...
export async function loginOrRegisterApple(
  ...
  const email =
    profile.email || `apple_${profile.sub}@privaterelay.agile-athletes.local`;
