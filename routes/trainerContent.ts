...
import { requireTrainer, type TrainerRequest } from '../middleware/trainer.ts';
import {
  createTrainerVideo,
  deleteTrainerVideo,
  ensureTrainerVideoIndexes,
  getTrainerVideoById,
  getTrainerVideoForTrainer,
  listAssignedVideosForMember,
  listAssignedVideosForMemberFromTrainer,
  listTrainerVideosForTrainer,
  updateTrainerVideo,
} from '../models/trainerVideo.ts';
import { getTrainerProfileById } from '../models/trainerProfile.ts';
import {
  buildTrainerVideoStorageKey,
  deleteTrainerVideoObject,
  getPublicApiBaseUrl,
  isTrainerVideoStorageReady,
  openLocalTrainerVideoStream,
  TRAINER_VIDEO_MAX_BYTES,
  uploadTrainerVideoObject,
  verifyTrainerVideoPlayToken,
} from '../services/trainerVideoStorage.ts';
import { routeParam } from '../utils/routeParams.ts';
import { toTrainerVideoClient, toTrainerVideoClients } from '../utils/trainerVideoResponse.ts';
import {
  canMemberPlayVideo,
...
function resolveApiBase(req: express.Request): string {
...
router.get('/me/content', authenticate, requireTrainer, async (req: TrainerRequest, res: Response) => {
  try {
...
router.post(
  ...
  authenticate,
  requireTrainer,
  (req: TrainerRequest, res: Response, next) => {
  ...
  async (req: TrainerRequest, res: Response) => {
    try {
      ...
      const file = (req as TrainerRequest & { file?: Express.Multer.File }).file;
...
router.get('/content/assigned', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/content/play/:videoId', async (req, res: Response) => {
  try {
    const videoId = routeParam(req.params.videoId);
...
router.get('/:trainerId/content', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
