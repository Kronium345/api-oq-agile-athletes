...
import { ensurePerformanceCheckinIndexes } from '../models/performanceCheckin.ts';
import {
  getPerformanceCheckInHistory,
  getPerformanceToday,
  getPerformanceTrends,
  getPerformanceWeeklySummary,
  resolveTrendPeriod,
  resolveWeekStart,
  submitPerformanceCheckIn,
} from '../services/performanceHub.ts';
import { isValidPerformanceDate, validateCheckInBody } from '../utils/performanceValidation.ts';
import { todayUtc } from '../utils/stepDates.ts';
...
router.post('/check-ins', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const parsed = validateCheckInBody(req.body as Record<string, unknown>);
    if (parsed.ok === false) {
...
router.get('/today', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const date =
      typeof dateRaw === 'string' && isValidPerformanceDate(dateRaw.trim())
        ? dateRaw.trim()
        : todayUtc();
...
router.get('/check-ins', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const startDate =
...
router.get('/trends', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/weekly-summary', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const weekStartRaw = req.query.weekStart;
    const weekStart = resolveWeekStart(
      typeof weekStartRaw === 'string' ? weekStartRaw.trim() : undefined
    );
