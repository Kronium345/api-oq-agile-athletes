...
const SIGNED_URL_TTL_SEC = Number(process.env.TRAINER_VIDEO_SIGNED_URL_TTL_SEC) || 60 * 60;
...
function isR2Configured() {
    return Boolean(process.env.R2_ACCOUNT_ID?.trim() &&
...
export function buildTrainerVideoStorageKey(trainerId, videoId, ext = '.mp4') {
    return `trainer-videos/${trainerId}/${videoId}${ext}`;
}
export function getTrainerVideoStorageProvider() {
    return resolveStorageProvider();
}
export function getPublicApiBaseUrl(fallbackHost) {
    const configured = process.env.TRAINER_VIDEO_API_BASE_URL?.trim() ||
...
export function signTrainerVideoPlayToken(videoId, userId) {
    return jwt.sign({ videoId, userId, purpose: 'trainer-video-play' }, getJwtSecret(), { expiresIn: SIGNED_URL_TTL_SEC });
}
export function verifyTrainerVideoPlayToken(token) {
    try {
        const payload = jwt.verify(token, getJwtSecret());
        if (payload.purpose !== 'trainer-video-play' ||
...
export async function uploadTrainerVideoObject(storageKey, body, contentType) {
...
export async function deleteTrainerVideoObject(storageKey, thumbnailKey) {
    const provider = resolveStorageProvider();
    if (provider === 'r2') {
        const client = getR2Client();
...
export async function getTrainerVideoPlayUrl(storageKey, videoId, userId, apiBaseUrl) {
    const provider = resolveStorageProvider();
    if (provider === 'r2') {
        const client = getR2Client();
...
export async function getTrainerVideoThumbnailUrl(thumbnailKey, videoId, userId, apiBaseUrl) {
    if (!thumbnailKey)
        return null;
    const provider = resolveStorageProvider();
    if (provider === 'r2') {
...
export async function openLocalTrainerVideoStream(storageKey) {
    const filePath = path.join(LOCAL_ROOT, storageKey);
...
export function isTrainerVideoStorageReady() {
    const provider = resolveStorageProvider();
    if (provider === 'local')
        return true;
