...
import { listPublishedTrainers } from "../models/trainerProfile.js";
import { generateCohereReply } from "./cohereChat.js";
import { geocodeUkPostcode } from "../utils/geocode.js";
import { toTrainerListItem } from "../utils/trainerResponse.js";
const GOAL_KEYWORD_SPECIALTIES = {
...
function specialtiesFromGoal(goal) {
...
function fallbackExplanations(trainers, goal) {
    return trainers.map((t) => `${t.displayName} at ${t.gymName} matches your goal "${goal}" with specialties in ${t.specialties.slice(0, 2).join(', ') || 'general fitness'}.`);
}
export async function matchTrainers(input) {
    const user = await getUserById(input.userId);
    const memberPostcode = input.postcode?.trim() ||
    ...
    if (inferredSpecialties.length) {
        const specialtyMatches = candidates.filter((t) => t.specialties.some((s) => inferredSpecialties.some((inf) => s.toLowerCase().includes(inf.toLowerCase()))));
        if (specialtyMatches.length) {
            candidates = specialtyMatches;
        }
    }
    const top = candidates.slice(0, 3);
    const trainers = top.map((t) => toTrainerListItem(t, { distanceKm: t.distanceKm }));
    let explanations = [];
    if (trainers.length && process.env.COHERE_API_KEY?.trim()) {
        try {
            const trainerSummary = trainers
                .map((t, i) => `${i + 1}. ${t.displayName} — ${t.gymName}, specialties: ${t.specialties.join(', ')}, rating: ${t.ratingAvg ?? 'N/A'}`)
                .join('\n');
            const memberContext = [
                `Goal: ${input.goal}`,
...
${trainerSummary}`;
            ...
            console.warn('[trainer-match] Cohere ranking failed, using fallback:', err.message);
        }
    }
