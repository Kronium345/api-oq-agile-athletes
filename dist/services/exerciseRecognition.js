 ...
 */
export const analyzeExerciseImage = async (imageBase64) => {
    ...
    try {
        ...
        const exerciseConcepts = concepts.filter(concept => {
            ...
            const matchesKeyword = exerciseKeywords.some(keyword => conceptName.includes(keyword) || keyword.includes(conceptName));
        ...
        const recognizedExercises = exerciseConcepts.map((concept, index) => generateExerciseData(concept, index));
...
const generateExerciseData = (concept, index) => {
...
const determineBodyPart = (name) => {
    ...
    if (name.includes('squat') || name.includes('lunge'))
        return 'legs';
    if (name.includes('press') || name.includes('pushup'))
        return 'chest';
    if (name.includes('curl'))
        return 'upper arms';
    if (name.includes('row') || name.includes('pullup'))
        return 'back';
 ...
 */
const determineTargetMuscle = (name, bodyPart) => {
    ...
    if (name.includes('shoulder'))
        return 'delts';
    if (name.includes('back') || name.includes('row'))
        return 'lats';
    if (name.includes('squat'))
        return 'quads';
    if (name.includes('deadlift'))
        return 'glutes';
    if (name.includes('ab') || name.includes('crunch'))
        return 'abs';
 ...
 */
const generateInstructions = (name, bodyPart, equipment) => {
 ...
 */
const getSecondaryMuscles = (bodyPart) => {
    const secondaryMap = {
