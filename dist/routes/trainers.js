...
import { listAvailableSlots, replaceTrainerSlots } from "../models/bookingSlot.js";
import { createTrainerLead, listLeadsForTrainer, updateLeadStatus, } from "../models/trainerLead.js";
import { createTrainerProfile, ensureTrainerProfileIndexes, getTrainerProfileById, getTrainerProfileByUserId, getTrainersByIds, listPublishedTrainers, recomputeTrainerRating, updateTrainerProfile, } from "../models/trainerProfile.js";
import { createTrainerReview, ensureTrainerReviewIndexes, getReviewByMember, getReviewsForTrainer, } from "../models/trainerReview.js";
import { addSavedTrainer, addTrainerRole, getUserById, removeSavedTrainer, } from "../models/user.js";
import { createAccountLink, getConnectStatus, getFrontendUrl, getOrCreateConnectAccount, getStripe, } from "../services/stripeConnect.js";
import { matchTrainers } from "../services/trainerMatch.js";
...
router.get('/saved', authenticate, async (req, res) => {
    try {
        ...
        const ids = user?.savedTrainerIds || [];
...
router.get('/leads', authenticate, requireTrainer, async (req, res) => {
    try {
...
router.put('/leads/:leadId', authenticate, requireTrainer, async (req, res) => {
    try {
...
router.post('/match', authenticate, async (req, res) => {
    try {
        const { goal, budget, postcode, trainingStyle, experience } = req.body;
        if (!goal?.trim()) {
...
router.get('/stripe-connect/status', authenticate, requireTrainer, async (req, res) => {
    try {
...
router.post('/stripe-connect/onboard', authenticate, requireTrainer, async (req, res) => {
    try {
        if (!getStripe()) {
...
router.get('/stripe-connect/callback', (_req, res) => {
...
router.post('/profile', authenticate, async (req, res) => {
    try {
        ...
        const profile = await createTrainerProfile({
            ...
            priceUnit: body.priceUnit || 'session',
            instagram: typeof body.instagram === 'string' ? body.instagram : undefined,
            availabilityNotes: typeof body.availabilityNotes === 'string' ? body.availabilityNotes : undefined,
...
router.put('/profile', authenticate, requireTrainer, async (req, res) => {
    try {
        const body = req.body;
        const updates = {};
        const stringFields = [
        ...
        if (typeof body.priceFrom === 'number')
            updates.priceFrom = body.priceFrom;
...
router.put('/availability', authenticate, requireTrainer, async (req, res) => {
    try {
...
router.get('/', async (req, res) => {
    try {
        ...
        const trainers = results.map((t) => toTrainerListItem(t, { distanceKm: t.distanceKm }));
...
router.post('/:id/contact-request', authenticate, async (req, res) => {
    try {
        const trainerId = routeParam(req.params.id);
        const trainer = await getTrainerProfileById(trainerId);
        if (!trainer?.published) {
        ...
        const { message, goal, budget } = req.body;
        if (!message?.trim()) {
