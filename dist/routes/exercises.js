...
router.get('/', authenticate, async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const result = await getAllExercises(userId, parseInt(limit, 10), lastKey ? JSON.parse(lastKey) : null);
        res.json({
            success: true,
            data: result.items,
            pagination: {
...
router.get('/stats/duration', authenticate, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const userId = req.userId;
        const totalDuration = await getTotalExerciseDuration(userId, startDate || null, endDate || null);
        res.json({
            success: true,
            data: { totalDuration },
        });
...
router.get('/stats/calories', authenticate, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const userId = req.userId;
        const totalCalories = await getTotalCaloriesBurned(userId, startDate || null, endDate || null);
        res.json({
            success: true,
            data: { totalCalories },
        });
