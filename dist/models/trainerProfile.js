...
export async function ensureTrainerProfileIndexes() {
    if (indexesEnsured)
        return;
    const col = getCollection();
    await col.createIndex({ userId: 1 }, { unique: true });
    await col.createIndex({ location: '2dsphere' });
    await col.createIndex({ displayName: 'text', gymName: 'text', specialties: 'text' }, { name: 'trainer_text_search' });
    await col.createIndex({ published: 1, featured: -1, ratingAvg: -1, createdAt: -1 });
    indexesEnsured = true;
}
export async function getTrainerProfileByUserId(userId) {
    return getCollection().findOne({ userId });
}
export async function getTrainerProfileById(trainerId) {
    return getCollection().findOne({ trainerId });
}
export async function createTrainerProfile(params) {
...
export async function updateTrainerProfile(userId, updates) {
...
export async function updateTrainerProfileById(trainerId, updates) {
    const updateDoc = { ...updates, updatedAt: new Date().toISOString() };
    await getCollection().updateOne({ trainerId }, { $set: updateDoc });
    return getTrainerProfileById(trainerId);
}
export async function listPublishedTrainers(query) {
    const col = getCollection();
    ...
    const useGeo = query.nearLat != null &&
        query.nearLng != null &&
        query.radiusKm != null &&
        query.radiusKm > 0;
    if (useGeo) {
...
export async function getTrainersByIds(trainerIds) {
...
export async function recomputeTrainerRating(trainerId) {
    const { getReviewsForTrainer } = await import("./trainerReview.js");
    const reviews = await getReviewsForTrainer(trainerId, 1000);
    const reviewCount = reviews.length;
    const ratingAvg = reviewCount > 0
        ? Math.round((reviews.reduce((s, r) => s + r.rating, 0) / reviewCount) * 10) / 10
        : undefined;
    await getCollection().updateOne({ trainerId }, { $set: { reviewCount, ratingAvg, updatedAt: new Date().toISOString() } });
}
