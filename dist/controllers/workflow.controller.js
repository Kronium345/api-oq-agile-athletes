...
import { sendDailyGoalReminderEmail, sendGoalAchievementEmail, sendLeaderboardAlertEmail, sendMotivationEmail, sendStreakMilestoneEmail, sendWeeklyProgressEmail, } from "../utils/send-email.js";
import { STREAK_MILESTONE_DAYS, checkGoalAchievements, fetchUserForWorkflow, fitnessUserPayload, getLeaderboardCompetitor, getTodayStepData, getWeeklyStepData, updateUserLastMotivation, userWantsEmail, } from "../services/workflowEmailData.js";
...
export const sendDailyStepReminders = serve(async (context) => {
    const userId = requireUserId(context);
    if (!userId)
        return;
    const user = await context.run('Get user', () => fetchUserForWorkflow(userId));
    if (!user || !userWantsEmail(user, 'stepReminders'))
        return;
...
export const sendWeeklyProgressSummary = serve(async (context) => {
    const userId = requireUserId(context);
    if (!userId)
        return;
    const user = await context.run('Get user', () => fetchUserForWorkflow(userId));
    if (!user || !userWantsEmail(user, 'weeklyProgress'))
        return;
...
export const sendLeaderboardAlerts = serve(async (context) => {
    const userId = requireUserId(context);
    if (!userId)
        return;
    const user = await context.run('Get user', () => fetchUserForWorkflow(userId));
    if (!user || !userWantsEmail(user, 'leaderboardAlerts'))
        return;
    const leaderboardData = await context.run('Get leaderboard data', () => getLeaderboardCompetitor(userId));
...
export const sendMotivationAndGoals = serve(async (context) => {
    const userId = requireUserId(context);
    if (!userId)
        return;
    const user = await context.run('Get user', () => fetchUserForWorkflow(userId));
    if (!user)
        return;
    const achievements = await context.run('Check goal achievements', () => checkGoalAchievements(userId));
