...
import { computeBreathingStreakDays, suggestBreathingProtocolId, } from "../services/recoveryHub.js";
import { isKnownProtocolId, listBreathingProtocols } from "../services/recoveryProtocols.js";
import { validateRecoverySessionBody } from "../utils/recoveryValidation.js";
...
describe('suggestBreathingProtocolId', () => {
    ...
    it('suggests post-workout when already completed today', () => {
        assert.equal(suggestBreathingProtocolId({ completedToday: true, stress: 8, hourUtc: 14 }), 'post_workout_recovery');
    });
});
describe('computeBreathingStreakDays', () => {
    ...
    it('returns 0 when today has no session', () => {
        assert.equal(computeBreathingStreakDays(['2026-07-14T08:00:00.000Z'], '2026-07-16'), 0);
    });
});
