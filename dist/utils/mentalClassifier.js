...
function parseFeatureValue(features, key) {
...
function scoreKeys(features, keys, maxPerItem) {
    let sum = 0;
    let count = 0;
 ...
 */
export function getPrediction(features) {
    ...
    let label;
    if (!hasAnger && !hasAnxiety)
        label = 0;
    else if (hasAnger && !hasAnxiety)
        label = 1;
    else if (!hasAnger && hasAnxiety)
        label = 2;
    else
