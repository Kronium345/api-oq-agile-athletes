export const generateFitnessEmailTemplate = (data) => `
...
export const emailTemplates = [
    {
        ...
        generateSubject: (d) => `🚶‍♀️ Don't break your ${d.streakDays}-day streak! Only ${Math.max(0, (d.dailyGoal ?? 0) - (d.currentSteps ?? 0))} steps to go!`,
        generateBody: (d) => generateFitnessEmailTemplate({ ...d, appLink: `${d.appLink}/(drawer)/(tabs)/stepCounter` }),
    },
    {
        label: 'Daily step goal reminder',
        generateSubject: (d) => `⏰ Evening reminder: ${d.stepsRemaining ?? 0} steps left to reach your goal!`,
        generateBody: (d) => generateFitnessEmailTemplate({ ...d, appLink: `${d.appLink}/(drawer)/(tabs)/stepCounter` }),
    },
    {
        label: 'Leaderboard alert',
        generateSubject: (d) => `🏆 ${d.friendName} is only ${d.stepsBehind} steps ahead! Time to catch up!`,
        generateBody: (d) => generateSimpleTemplate({
            userName: d.userName,
    ...
    },
    {
        ...
        generateSubject: (d) => `🏃‍♀️ Reminder: "${d.runName}" starts soon at ${d.runLocation}!`,
        generateBody: (d) => generateSimpleTemplate({
            userName: d.userName,
    ...
    },
    {
        ...
        generateSubject: (d) => `📊 Your weekly summary: ${(d.weeklySteps ?? 0).toLocaleString()} steps!`,
        generateBody: (d) => generateFitnessEmailTemplate({ ...d, appLink: `${d.appLink}/(drawer)/(tabs)/stepCounter` }),
    },
    {
        ...
        generateBody: (d) => generateSimpleTemplate({
            userName: d.userName,
            message: 'Every step and every healthy choice matters. You are building habits that last! 🌟',
