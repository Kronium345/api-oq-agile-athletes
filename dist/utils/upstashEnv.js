...
export function getQstashConfig() {
    return {
        ...
        hasSigningKeys: Boolean(process.env.QSTASH_CURRENT_SIGNING_KEY?.trim() &&
            process.env.QSTASH_NEXT_SIGNING_KEY?.trim()),
    };
}
export function logQstashStartup() {
    ...
    if (!hasSigningKeys) {
        console.warn('[qstash] QSTASH_CURRENT_SIGNING_KEY and QSTASH_NEXT_SIGNING_KEY not set — workflow POSTs are not signature-verified');
    }
}
